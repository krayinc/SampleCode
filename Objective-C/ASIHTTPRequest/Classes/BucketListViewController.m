...
- (void)viewWillAppear:(BOOL)animated {
  ...
  if (![request error]) {
    buckets = [request.buckets retain];
  } else {
    buckets = [NSArray new];
  }
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
  static NSString *CellIdentifier = @"S3BucketCell";
  ...
  // Configure the cell...
  ASIS3Bucket *bucket = [buckets objectAtIndex:indexPath.row];
  cell.textLabel.text = bucket.name;
  
  NSDateFormatter *fmt = [[NSDateFormatter alloc] init];
  [fmt setDateStyle:NSDateFormatterMediumStyle];
  [fmt setTimeStyle:NSDateFormatterMediumStyle];
  cell.detailTextLabel.text = [fmt stringFromDate:bucket.creationDate];
  [fmt release];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
  ...
  vc.bucket = [buckets objectAtIndex:indexPath.row];
  vc.path = @"";
  // ...
